        ...
        <nav className="fixed top-0 left-0 right-0 z-50 bg-gray-950/80 backdrop-blur-xl border-b border-gray-800">
            ...
            {menuOpen && (
                <div className="md:hidden bg-gray-950 border-t border-gray-800 px-6 py-4 max-h-96 overflow-y-auto">
                    <Link href="/" className="block text-gray-300 text-sm py-2" onClick={() => setMenuOpen(false)}>Home</Link>
                    <div className="text-xs text-gray-600 uppercase tracking-wider mt-3 mb-2">Projects</div>
                    {projects.map(p => (
                        <Link
                            key={p.id}
                            href={`/projects/${p.slug}`}
